...
func _on_load_scene_finished() -> void:
	...
	await get_tree().physics_frame #twice, wait 2 frames
	await get_tree().physics_frame #avoid overlapping transitions
...
func get_offset(player : Node2D) -> Vector2:
	...
	if location == SIDE.LEFT or location == SIDE.RIGHT:
		...
		if location == SIDE.LEFT:
			offset.x = -12  #derived from player's collider size
		else:
			offset.x = 12
	else:
